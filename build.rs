//! Copies `template/this_project` into `$OUT_DIR/template`, skipping files that must
//! never ship inside the binary (private profiles, maintainer docs, superseded files,
//! OS cruft). `src/template.rs` then embeds the filtered copy with `include_dir!`.
//!
//! This denylist is one of two layers: a unit test in `src/template.rs` additionally
//! asserts that every embedded file is git-tracked, so an untracked private file in a
//! maintainer's checkout fails `cargo test` instead of shipping silently.
...
fn main() {
    ...
    // Start clean so files deleted from the source template also disappear from the embed.
