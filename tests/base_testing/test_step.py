...
from exporgo.organization.step import (
    Category,
    RegisteredStep,
    Status,
    Step,
...
class TestValidStep:
    ...
    def test_validate_call_with_dict(self):
        result = self.test_step_case.validate_call(
            serialize_function(generic_function_call)
        )
        assert (
            inspect.signature(result).parameters
            == inspect.signature(generic_function_call).parameters
        )
...
class TestRegisteredStep:
    ...
    base_category = Category.ANALYZE
    base_status = Status.SOURCE
    test_registered_step_case = RegisteredStep(
        key=test_key,
        call=generic_function_call,
        file_sets=test_file_sets,
        category=base_category,
    ...
    def test_validate_category(self):
        result = self.test_registered_step_case.validate_category(
            Category.ANALYZE.value
        )
        assert result == Category.ANALYZE
...
class TestStepRegistry:
    ...
    @pytest.mark.parametrize(
        "collection_type", [list, tuple, set, create_step_generator]
    )
    ...
    def test_exit_saves_registry_on_new_registration(self):
        with patch.object(
            ...
            with StepRegistry() as registry:
                registry.register(
                    self.create_step("test_exit_saves_registry_on_new_registration")
                )
            # noinspection PyUnresolvedReferences
