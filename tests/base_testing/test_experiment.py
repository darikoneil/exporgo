...
from exporgo.exceptions import DuplicateRegistrationError, ExperimentNotRegisteredError
from exporgo.organization.experiment import (
    Experiment,
    ExperimentFactory,
    ExperimentRegistry,
...
class TestValidExperiment:
    ...
    def test_serialize_parent_directory(self):
        serialized_parent_directory = ValidExperiment.serialize_parent_directory(
    ...
    def test_validate_file_tree(self, tmp_path):
        _ = ValidExperiment.validate_file_tree(
            FileTree(tmp_path, "files").__serialize__()
        )
    ...
    def test_validate_priority(self):
        with patch(
            "exporgo.organization.experiment.Priority.__deserialize__",
            return_value=Priority.NORMAL,
        ) as mock_validate_priority:
            _ = ValidExperiment.validate_priority(Priority.NORMAL.__serialize__())
            mock_validate_priority.assert_called_once_with(
                Priority.NORMAL.__serialize__()
            )

    def test_validate_status(self):
        with patch(
            "exporgo.organization.experiment.Status.__deserialize__",
            return_value=Status.SOURCE,
        ) as mock_validate_status:
...
class TestMockExperiment:
    ...
    def test_add_sources_updates_pipeline_sources_dispatched_one_arg(self, tmp_path):
        ...
        self.experiment.pipeline.add_source.assert_called_once_with(
            "files", "new_source"
        )
    ...
    def test_collect_calls_pipeline_collect(self):
        ...
        self.experiment.pipeline.collect.assert_called_once_with(
            self.experiment.file_tree
        )
...
class TestIntegratedExperiment:
    ...
    def test_experiment_created(self, tmp_path):
        with patch(
            "exporgo.organization.experiment.get_timestamp", return_value="Timestamp"
        ) as get_timestamp_:
...
class TestRegisteredExperiment:
    ...
    @pytest.mark.parametrize(
        "additional_file_sets",
        [None, ["additional_files"], ["files", "additional_files"]],
    )
...
class TestExperimentRegistry:
    ...
    def setup_class(self, path_steps, path_experiments):
        ...
        StepRegistry._StepRegistry__registry = {
            "generic_step_0": RegisteredStep(
                key="generic_step_0",
                call=generic_function_call,
                file_sets="files",
                category=Category.ANALYZE,
            ),
            "generic_step_1": RegisteredStep(
                key="generic_step_1",
                call=generic_function_call,
                file_sets="files",
                category=Category.ANALYZE,
    ...
    def create_experiment(key: str) -> RegisteredExperiment:
        return RegisteredExperiment(
            ...
            pipeline=RegisteredPipeline(
                steps=[
                    StepRegistry.get("generic_step_0"),
    ...
    def test_has_returns_true_for_registered_key(self):
        ...
        # noinspection PyUnresolvedReferences
        ExperimentRegistry._ExperimentRegistry__registry[key] = self.create_experiment(
            key
        )
    ...
    def test_has_returns_false_for_unregistered_key(self):
        assert (
            ExperimentRegistry.has("test_has_returns_false_for_unregistered_key")
            is False
        )
    ...
    @pytest.mark.parametrize(
        "collection_type", [list, tuple, set, create_experiment_generator]
    )
    ...
    def test_enter_loads_registry(self):
        with patch.object(
            ExperimentRegistry,
            "_load_registry",
            MagicMock(ExperimentRegistry._load_registry),
        ):
    ...
    def test_exit_saves_registry_on_new_registration(self):
        with patch.object(
            ExperimentRegistry,
            "_save_registry",
            MagicMock(ExperimentRegistry._save_registry),
        ):
            with ExperimentRegistry() as registry:
                registry.register(
                    self.create_experiment(
                        "test_exit_saves_registry_on_new_registration"
                    )
                )
            # noinspection PyUnresolvedReferences
...
class TestExperimentFactory:
    ...
    def test_create_experiment_with_valid_data(self, tmp_path):
        with patch.object(
            ExperimentRegistry,
            "get",
            return_value=self.create_experiment(
                "test_create_experiment_with_valid_data"
            ),
        ):
    ...
    def test_enter_loads_experiment_registry(self, tmp_path):
        with patch.object(
            ...
            with ExperimentFactory(
                name="test_experiment", parent_directory=tmp_path
            ) as factory:
                ...
            # noinspection PyUnresolvedReferences
            assert ExperimentRegistry._load_registry.called
