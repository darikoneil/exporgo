...
{meth}`~exporgo.experiment.Experiment.validate` asks *is what I registered still there?*
{meth}`~exporgo.experiment.Experiment.coverage` asks *which identities does each component actually
...
A **closed-world** view starts from the registry (the identities you declared the experiment should
...
{meth}`~exporgo.experiment.Experiment.validate` walks every registered identity and checks that the files
...
out of scope here: they hold exporgo-owned data, not files the experiment merely points at, so their
membership is a coverage concern. The report holds no live handles, so it's safe to store or
diff across runs.

## `coverage`: membership plus drift

{meth}`~exporgo.experiment.Experiment.coverage` generalizes `validate` across resources, stores, and array
...
{meth}`~exporgo.experiment.CoverageReport.to_polars` reshapes the report into a tidy long frame: one
...
{meth}`~exporgo.experiment.Experiment.discover` is for: it reverse-resolves each resource template to
...
folded into the registry, bootstrapping an experiment from data that already exists. See
...
store manifests) at call time, every time, and a loaded experiment re-derives status rather than
