...
A study has five kinds of component, and most of using exporgo well is reaching for the right
one. This page routes you there. Read [Resources, stores, file maps, and dumps](./components.md) first
...
location known?** exporgo either *reads* data it doesn't own (resources, file maps, dumps) or
*owns and writes* it (tabular stores, array stores). Within each half, the next question is
shape — one path or a folder of files, a table or a single array, per-identity or study-global.
...
| **File map** | exporgo indexes; you own | **recorded index**: templated root *or* recorded dir/`record` | a folder of many files per identity, keyed by relative path | files stay put; index in `<root>/<name>/_filemap.json` | {meth}`~exporgo.study.FileMap.path` / {meth}`~exporgo.study.FileMap.paths` → {class}`~pathlib.Path` / `{key: Path}` | `validate` + `coverage` |
...
1. **Does exporgo write the bytes, or do you?** If the data is produced elsewhere and exporgo
   only needs to find it and read it, you want a **resource**, a **file map**, or a **dump** —
   skip to question 3. If exporgo should *own* the data and write it under a strict schema,
   continue.
...
4. **One predictable path, or a folder of many files?** A single file or folder you can name with
   a `{Key}` pattern → a **resource** ({meth}`~exporgo.study.Study.declare_resource`). A whole
   directory of files that must be addressed individually → a **file map**
   ({meth}`~exporgo.study.Study.declare_filemap`).
5. **(File map only) Is the layout predictable?** If each identity's folder sits at a templated
   location, pass a `root_template` for a **templated** file map. If the files land wherever the
   scope or the tech dropped them, omit it for a **recorded** file map and hand `discover` the
   folder (or pin loose files with {meth}`~exporgo.study.FileMap.record`).
...
- **A folder of files with no single natural path → file map.** A suite2p output tree, an
  external drive full of raw scope files.
...
them in a resource or file map rather than copying them into a store.
