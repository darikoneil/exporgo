...
use crate::template;
use crate::tokens::{self, Token, TokenValues};
...
pub struct NewOptions {
    ...
    /// Overwrite colliding files in a non-empty target (never wipes the target).
    pub force: bool,
...
///
/// Guards: refuses a non-empty target without `force`, and refuses a target that
/// already contains an `exporgo.toml` even with `force` (that calls for
...
pub fn stamp(options: &NewOptions) -> Result<StampReport, Error> {
    ...
    if is_windows_reserved(&slug) {
        // CON, NUL, COM1… cannot be created as directories on Windows; fail with
        // a clear message instead of a raw I/O error.
