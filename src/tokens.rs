//! `{{TOKEN}}` placeholders in the template's `context.md`, and name slugification.
//!
//! The token set is a hardcoded enum because each token needs a static CLI flag,
//! prompt text, and manifest key. A unit test asserts the enum matches the
//! placeholders actually present in the embedded `context.md`, so the template and
...
impl Token {
    ...
    /// The literal placeholder as it appears in `context.md`, e.g. `{{PROJECT_NAME}}`.
...
/// All `{{UPPER_SNAKE}}` placeholders present in `text`, whether or not they are
/// known [`Token`]s (users may add their own to a project's `context.md`).
pub fn scan(text: &str) -> BTreeSet<String> {
    ...
    while let Some(start) = text[i..].find("{{").map(|p| p + i) {
        ...
        let Some(end) = text[body_start..].find("}}").map(|p| p + body_start) else {
            break;
        };
...
///
/// A single left-to-right pass: substituted values are emitted verbatim and never
/// rescanned, so a value that itself contains a placeholder (e.g. an aim quoting
/// `{{OWNER}}`) is not substituted recursively.
pub fn substitute(text: &str, values: &TokenValues) -> String {
    let mut result = String::with_capacity(text.len());
    let mut rest = text;
    while let Some(start) = rest.find("{{") {
        let after_open = start + 2;
        let Some(close) = rest[after_open..].find("}}").map(|p| p + after_open + 2) else {
            break;
        };
